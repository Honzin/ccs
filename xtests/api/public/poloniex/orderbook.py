...
"""
This file implements test cases for validation communication with REST endpoint on Poloniex. This endpoint offer informations about orderbook.
"""
...
def schema():
    return ccs.poloniex.configuration.SCHEMA["returnOrderBook"]
...
class Valid(unittest.TestCase):
    ...
    def testSchema(self):
        r = ccs.poloniex.public.returnOrderBook(self.pair)
        jsonschema.validate(json.loads(r), schema())


    def testDepth(self):
        depth = 10
        r = ccs.poloniex.public.returnOrderBook(self.pair, depth=depth)
        d = json.loads(r)
        jsonschema.validate(d, schema())

        self.assertLessEqual(len(d["asks"]), depth)
        self.assertLessEqual(len(d["bids"]), depth)

    @unittest.skip("time-consuming")
    def testPairs(self):
        for pair in self.pairs:
            r = ccs.poloniex.public.returnOrderBook(pair)
            jsonschema.validate(json.loads(r), schema())
            sleep()
