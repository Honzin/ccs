...
def schema():
    return ccs.btccusd.configuration.SCHEMA["tradeHistory"]
...
class Valid(unittest.TestCase):
    ...
    def testSchema(self):
        r = ccs.btccusd.public.tradeHistory(self.market)
        jsonschema.validate(json.loads(r), schema())

    def testLimit(self):
        l = 2
        r = ccs.btccusd.public.tradeHistory(self.market, limit=l)
        d = json.loads(r)
    ...
    def testSinceId(self):
        s = 7000
        r = ccs.btccusd.public.tradeHistory(self.market, since=s)
        d = json.loads(r)
        jsonschema.validate(d, schema())
    ...
    def testSinceTime(self):
        s = 1484396000
        st = "time"
        r = ccs.btccusd.public.tradeHistory(self.market, since=s, sincetype=st)
        d = json.loads(r)
        jsonschema.validate(d, schema())
    ...
    def testCurrencies(self):
        for market in self.markets:
            r = ccs.btccusd.public.tradeHistory(market)
            jsonschema.validate(json.loads(r), schema())
            sleep()
