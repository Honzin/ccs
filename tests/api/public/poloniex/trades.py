...
"""
This file implements test cases for validation communication with REST endpoint on Poloniex. This endpoint offer informations about trades.
"""
...
def schema():
    return ccs.poloniex.configuration.SCHEMA["returnTradeHistory"]
...
class Valid(unittest.TestCase):
    ...
    def testSchema(self):
        r = ccs.poloniex.public.returnTradeHistory(self.pair)
    ...
    def testStartEnd(self):
        s = 1410158341
        e = 1410499372
        r = ccs.poloniex.public.returnTradeHistory(self.pair, start=s, end=e)
        d = json.loads(r)
        jsonschema.validate(d, schema())
        dts = datetime.datetime.fromtimestamp(s)
        dte = datetime.datetime.fromtimestamp(e)

        dtds = datetime.datetime.strptime(d[0]["date"], "%Y-%m-%d %H:%M:%S")
        dtde = datetime.datetime.strptime(d[-1]["date"], "%Y-%m-%d %H:%M:%S")

        self.assertGreaterEqual(dtds, dts)
        self.assertLessEqual(dtde, dte)

    @unittest.skip("time-consuming")
    def testPairs(self):
        for pair in self.pairs:
            r = ccs.poloniex.public.returnTradeHistory(pair)
            jsonschema.validate(json.loads(r), schema())
            sleep()
