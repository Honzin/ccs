...
"""
This file implements test cases for validation communication with REST endpoint on Cexio. This endpoint offer informations about last price.
"""
...
def schema():
    return ccs.cexio.configuration.SCHEMA["lastPrice"]
...
class Valid(unittest.TestCase):
    ...
    def testCurrencies(self):
        for c1, c2 in zip(self.currencies1, self.currencies2):
            r = ccs.cexio.public.lastPrice(c1, c2)
            jsonschema.validate(json.loads(r), schema())
            sleep()
...
class Invalid(unittest.TestCase):
    ...
    def testResponse(self):
        with self.assertRaises(jsonschema.exceptions.ValidationError):
            r = ccs.cexio.public.lastPricesForGivenMarket(self.curr1, self.curr2)
            jsonschema.validate(json.loads(r), schema())
