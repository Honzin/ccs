...
"""
This file implements test cases for validation communication with REST endpoint on Bittrex. This endpoint offer informations about orderbook.
"""
...
def schema():
    return ccs.bittrex.configuration.SCHEMA["getOrderbook"]
...
class Valid(unittest.TestCase):
    ...
    def testDepth(self):
        depth = 10
        r = ccs.bittrex.public.getOrderbook(self.market, depth=depth)
        d = json.loads(r)
        jsonschema.validate(d, schema())

        self.assertLessEqual(len(d["buy"]), depth)
        self.assertLessEqual(len(d["sell"]), depth)


    def testTypeSell(self):
        r = ccs.bittrex.public.getOrderbook(self.market, type="sell")
        d = json.loads(r)
        s = ccs.bittrex.configuration.SCHEMA["getOrderbookBuySell"]
        jsonschema.validate(d, s)


    def testTypeBuy(self):
        r = ccs.bittrex.public.getOrderbook(self.market, type="buy")
        d = json.loads(r)
        s = ccs.bittrex.configuration.SCHEMA["getOrderbookBuySell"]
        jsonschema.validate(d, s)

    def testTypeBoth(self):
        r = ccs.bittrex.public.getOrderbook(self.market, type="both")
        d = json.loads(r)
        s = ccs.bittrex.configuration.SCHEMA["getOrderbook"]
        jsonschema.validate(d, s)
